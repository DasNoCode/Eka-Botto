...
class Command(BaseCommand):
    ...
    def __init__(self, client, handler):
        super().__init__(client, handler, {
            ...
            'category': 'core',
            'description': {
                'content': 'Identify the music playing around you'
            },
            'exp': 1
        })
    ...
    async def exec(self, M: Message, contex):
        ...
        recognize = lambda path: Serialize.full_track(shazam.recognize(path))
        ...
        if str(M.media).split(".")[-1] == "AUDIO" :
            ...
            await self.client.send_photo(M.chat_id,r.track.sections[0].meta_pages[1].image,f"Name: {r.track.title}\nReleased: {r.track.sections[0].metadata[2].text}")
